...
{ \
	static INTERFACE*& pInstance = reinterpret_cast<INTERFACE*&>(ISECore::Get()->Activate(reinterpret_cast<INSTANCE*>(INTERFACE::Entity())->Init())); \
	return pInstance; \
} \
\
INTERFACE* INTERFACE::Entity() /*获取接口实体。*/  \
{ \
	static INTERFACE*& pInstance = reinterpret_cast<INTERFACE*&>(ISECore::Entity()->Awake(new INSTANCE())); \
	return pInstance; \
} \
